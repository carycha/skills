...
- **大規模優化策略 (Bulk Optimization)**：
    - 當技能數量 > 50 時，應撰寫 Python 腳本結合 `PyYAML` 進行批量正則替換。
    - 腳本應能解析 YAML frontmatter，提取 `name` 作為專家角色的基礎。
    - 自動化規則：若描述為空或過短，以 `[Name] Expert. Use when the user asks to interact with or manage [Name]. 「[Name]」` 作為保底模板。
