...
- **路徑與掃描優化**：在移植含 Bash 腳本的技能（如 `skill-stocktake`）時，將 `~/.claude` 路徑映射為 `~/.gemini`，並在 `find` 指令中加入 `-maxdepth 2` 以精確匹配 `skill-name/SKILL.md` 結構，避免掃描到輔助檔案。
- **模式型技能分離策略**：對於純架構或設計模式技能（如 `content-hash-cache-pattern`），應將「具體代碼實作」與「設計權衡分析」分離至 `references/` 目錄。`SKILL.md` 僅保留「觸發情境」與「核心邏輯步驟」，以極大化初始 Context 的利用率。
