...
scan_dir_to_json() {
  ...
  local obs_7d_counts=""
  local obs_30d_counts=""
  if [[ -f "$OBSERVATIONS" ]]; then
    obs_7d_counts=$(jq -r --arg c "$c7" 'select(.timestamp>=$c) | .path' "$OBSERVATIONS" 2>/dev/null | sort | uniq -c || true)
    obs_30d_counts=$(jq -r --arg c "$c30" 'select(.timestamp>=$c) | .path' "$OBSERVATIONS" 2>/dev/null | sort | uniq -c || true)
  fi
